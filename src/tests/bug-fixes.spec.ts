...
test('#22 - Broken Order/Render From URL Import', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: '10.4.0.0/14 Join' })).toContainText('/14');
  await expect(page.getByRole('cell', { name: '10.0.0.0/11 Join' })).toContainText('/11');
  await expect(page.getByRole('cell', { name: '10.0.0.0/10 Join' })).toContainText('/10');
  await expect(page.getByRole('cell', { name: '10.0.0.0/9 Join' })).toContainText('/9');
  await expect(page.getByRole('cell', { name: '/15 Join' })).toContainText('/15');
  await expect(page.getByRole('cell', { name: '/13 Join' })).toContainText('/13');
  await expect(page.getByRole('cell', { name: '/8 Join' })).toContainText('/8');
});
