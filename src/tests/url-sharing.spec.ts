...
test('Reddit Example URL Conversion (URL v1 - Config v1 to v2)', async ({ page }) => {
  ...
  await page.getByText('Copy Shareable URL').click();
  const clipboardUrl = await getClipboardText(page);
  expect(clipboardUrl).toContain('/index.html?c=-z+oHYgIAHIc8WCIVCYfDohdYFDGq90eJMXSGIROTR8TyifzYhAHkgqbxxbTviBBLjZdzCTDgYjkai8uiJKQpHTscyQORdSD9byAkbomTKWbqRaraUAOyA9TsGWOgnO12XIUNdae3je6TDQTBkByg0uhYxZVgVUxsRx630oZAA');
});
